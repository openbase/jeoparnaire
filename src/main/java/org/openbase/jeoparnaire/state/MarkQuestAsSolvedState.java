 ...
 */
public class MarkQuestAsSolvedState extends AbstractGameState {

    @Override
    public Class<? extends State> call() throws CouldNotPerformException {
        gameController.getActiveQuest().setSolved(true);
        if (gameController.getData().isQuestAvailable()) {
            return WaitForQuestState.class;
        }
        return GameOverState.class;
    }
}
